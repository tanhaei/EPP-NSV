...
| An independent oracle and mutation testing corroborate verifier fault detection. | **Supported (synthetic).** `epp_nsv.independent_oracle` and `epp_nsv.mutation` provide a non-shared oracle and single-point policy mutants scored through the verifier; see `docs/MUTATION_TESTING.md`. | Mutation operators reviewed against the clinical policy, and a clinician-adjudicated reference standard for any clinical-correctness claim. |
